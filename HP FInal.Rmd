...
Common spell incantation patterns include:

- **Protection spells**: Often end in "-o" (e.g., "Protego", "Expecto Patronum")
- **Attack spells**: Frequently begin with "Ex-" (e.g., "Expelliarmus", "Expulso")
- **Charm spells**: Commonly use Latin-based words (e.g., "Lumos", "Wingardium Leviosa")
...
SpellCountByChapter<-SpellCountByCharacter %>% 
  ...
  group_by(Spell) %>% 
  filter(sum(CastsPerChapter)>2 && n_distinct(Chapter.ID)>1)
...
- Data completeness: Not all characters have complete wand or spell information recorded
- Context limitation: The analysis cannot account for the circumstances or effectiveness of spell usage
- Characteristic gaps: Some characters in the series have more detailed documentation than others
- Character development: The analysis may not fully capture how characters' magical abilities evolve over time

These findings provide valuable insights into the structured nature of magic in the Harry Potter universe while acknowledging the constraints of available data and analytical methods.
