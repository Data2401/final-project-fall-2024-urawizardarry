...
```{r message = FALSE, warning=FALSE}
SpellFinderDF<-SpellFinder(Dialogue, Spells)
SpellUsage<-left_join(Dialogue, SpellFinderDF, multiple="first") %>% filter(!is.na(Spell)) %>% 
  arrange(Dialogue.ID)


SpellsByCharacter<-left_join(SpellUsage,Characters, by="Character.ID")



SpellCountByCharacter<-SpellsByCharacter %>% 
  group_by(Character.Name, Spell,Chapter.ID,Character.ID) %>% 
